...
![Versão do App](https://img.shields.io/badge/versão-0.3.0-blueviolet?style=for-the-badge)
...
### `v0.3.0` — Navegação Interativa e Expansão de Interfaces *(atual)*

Esta versão adiciona um conjunto completo de novas telas, rotas de navegação interativas e feedback visual de toque em todos os principais botões da aplicação.

**Novas funcionalidades e telas:**
- [x] **Tela de Notificações (`NotificationsPage`)**: Ícone de sino do topo da Home direciona para o painel de notificações com filtros por aba (*Todas*, *Dispositivo*, *Aplicativos*).
- [x] **Tela de Configurações (`SettingsPage`)**: Central completa de preferências estruturada em 4 módulos (*Conectividade e Dispositivo*, *Preferências das Funções*, *Notificações e Alertas*, *Conta e Suporte*). Acessível via Home e pela aba Perfil.
- [x] **Tela de Idiomas (`LanguagePage`)**: Seleção interativa entre 13 idiomas globais com sinalizadores gráficos e suporte a localização.
- [x] **Tela de Controles de Áudio (`AudioControlsPage`)**: Painel com ajuste fino de volume, botões de mudo/microfone, modos de áudio (*Transparência*, *Cancelamento de Ruído*, *Melhoria de Voz*) e equalizador de som.
- [x] **Tela de Info. da Bateria (`BatteryPage`)**: Substitui o antigo atalho de notificações no menu rápido. Apresenta indicador gráfico de 85% de carga, gráfico de consumo por aplicativo, alternadores de modo de energia e diagnósticos de saúde da bateria.
- [x] **Tela de Histórico Mensal (`CalendarPage`)**: Botão de calendário no topo da seção *Atividades* direciona para a visualização completa do calendário mensal com codificação de cores por nível de uso (Intenso, Moderado, Sem Uso, Dia Atual), médias diárias e estatísticas de maior uso do mês.
- [x] **Feedback Visual de Toque**: Efeitos táteis e visuais (*InkWell ripples*, *splash colors*, animações de escala e transição) aplicados a todos os botões e cards interativos.
- [x] Análise estática de código (`flutter analyze`) com **0 erros e 0 warnings**.

### `v0.2.0-alpha` — Reformulação Visual e de Acessibilidade
...
├── core/
│   ├── constants/
│   │   ├── app_constants.dart                       # Constantes gerais do app (v0.3.0)
│   │   └── ble_constants.dart                       # UUIDs e configurações BLE
│   ├── mock/
│   │   └── app_mock_data.dart                       # Dados mock atualizados
│   └── utils/
...
└── features/
    ...
    │   └── presentation/
    │       └── pages/
    │           ├── home_page.dart                   # Dashboard principal com navegação
    │           ├── notifications_page.dart          # [NOVO v0.3.0] Painel de notificações por abas
    │           ├── settings_page.dart               # [NOVO v0.3.0] Central de configurações
    │           ├── language_page.dart               # [NOVO v0.3.0] Seleção de idiomas
    │           ├── audio_controls_page.dart         # [NOVO v0.3.0] Controles de volume e equalizador
    │           └── battery_page.dart                # [NOVO v0.3.0] Info. detalhada da bateria
    ├── connection/
    │   ├── domain/
    │   └── presentation/
    │       └── pages/
    ...
    │   └── presentation/
    │       └── pages/
    │           ├── activities_page.dart             # Gráfico semanal e resumo de uso
    │           └── calendar_page.dart               # [NOVO v0.3.0] Histórico Mensal e calendário
    └── profile/
        └── presentation/
            └── pages/
                └── profile_page.dart               # Perfil, acessibilidade e atalho para conta
```

## 🎨 Design System & Acessibilidade

O aplicativo segue diretrizes rigorosas de acessibilidade:

| Token / Métrica | Valor | Descrição |
|---|---|---|
| `buttonMinHeight` | `56 dp` | Alvos de toque mínimos (WCAG 2.1 AA) |
| Contraste de cor | `≥ 4.5:1` | Contraste mínimo para texto normal |
| `primaryColor` | `#6FA8DC` | Azul pastel acessível |
| `darkBackground` | `#121820` / `#0F172A` | Fundo dark otimizado para OLED/contrast |
| Tipografia | **Inter / Outfit** | Fontes limpas do Google Fonts |
| Feedback Tátil | InkWell Ripple | Resposta tátil ao clique de botões |
...
| `fl_chart` | ^0.69.0 | Gráficos de atividade semanal e rosca |
...
## 📋 Como Testar no Celular e Versionar no GitHub

### Testando no Celular (Dispositivo Físico)
1. Ative o **Modo Desenvolvedor** e a **Depuração USB** no seu smartphone Android.
2. Conecte o dispositivo ao computador via cabo USB.
3. Certifique-se de que o dispositivo é reconhecido executando:
   ```bash
   flutter devices
   ```
4. Execute o aplicativo diretamente no celular:
   ```bash
   flutter run
   ```
...
| Navegação entre abas e telas | Dispositivo / Widget | ✅ **Passed** |
